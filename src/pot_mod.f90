	...
	real*8 function calc_pot(model,nosc,pos_osc,kforce,coup_const,de,req)
		...
		real*8::energy,aux,a
                ...
                if (model=='har') then
                        ...
                        do i=2,nosc
                                energy=energy+0.5d0*kforce(i)*pos_osc(i)**2+coup_const*pos_osc(i)*pos_osc(1) !Simplified interaction hamiltonian
                        enddo
                        ...
                        aux=0.d0
                        do i=2,nosc
                                energy=energy+coup_const/kforce(1)*pos_osc(i)
                        ...
                        aux=0.d0
                        do i=2,nosc
                                energy=energy+coup_const/kforce(1)*pos_osc(i) !This is the dip moment squared parenthesis.
...
!                                       write(6,*) 'energy subroutine',i,energy
                                energy=energy+coup_const*pos_osc(i)*pos_osc(1) !Adding interaction with the cavity
                        enddo
                        ...
                        do i=2,nosc
                                ...
                                if (pos_osc(i)>0.d0) then
                                        aux=aux+de*(1.d0-dexp(a*(pos_osc(i)-req)))**2
                                        energy=energy+coup_const/kforce(i)*pos_osc(i)
                                else
		...
		real*8,intent(in)::coup_const,de,req
		real*8,dimension(nosc),intent(in)::kforce
		...
		real*8::sumpos,a
                ...
                if (model=='har') then
                                ...
                                sumpos=0.d0
                                do i=2,nosc
                                        grad(i)=grad(i)+coup_const*pos_osc(1)
                                        sumpos=coup_const*pos_osc(i)
                                enddo
                        ...
                        do i=2,nosc
                                grad(i)=grad(i)+coup_const*pos_osc(1)
                                sumpos=sumpos+pos_osc(i)
                        enddo
                        grad(2:)=grad(2:)+coup_const**2/kforce(1)*sumpos
                        grad(1)=grad(1)+coup_const*sumpos
                        ...
                        sumpos=0.d0
                        do i=2,nosc
                                grad(i)=coup_const*pos_osc(1)+kforce(i)*pos_osc(i)
                                sumpos=sumpos+pos_osc(i)
                        enddo
                        grad(2:)=grad(2:)+coup_const**2/kforce(1)*sumpos
                        grad(1)=grad(1)+coup_const*sumpos
                        ...
                        do i=2,nosc
                                grad(i)=grad(i)+(coup_const**2/kforce(1))*sumpos
                        enddo
                        ...
                        do i=2,nosc
                        sumpos=sumpos+pos_osc(i) !This may make the total dipole moment equal to 0.
                                grad(i)=grad(i)+coup_const*pos_osc(1)
                        enddo
                        ...
                        do i=2,nosc
                                grad(i)=grad(i)+(coup_const**2/kforce(i))*sumpos
                        enddo
