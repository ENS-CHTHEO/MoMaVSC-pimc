program path_montecarlo
	...
	real*8::coup_const=0.d0,rabi_split=0.d0
        real*8,dimension(:),allocatable::coup_list
	...
	logical::verbose=.False. !Controls if detailed data files are written or not.
        ...
        logical::outlier=.False. !Allows to introduce an outlier frequency associated to the 2nd oscillator (first matter one)
        real*8::outlier_freq
	...
	real*8,dimension(:),allocatable::history,work !Stores action values for each iteration
        ...
        integer::print_it
        !Namelist
        namelist /seetings/ model,rabi_split,nosc,nbeads,temp,mass_1,mass_2,w_oh,cycles,step,&
                start_sampling,estimator_interval,freq_coll,nblock1,nblock2,nblock3,verbose,width_freq,&
                w_cav,de,outlier,outlier_freq,load_rand,configs
        read(5,seetings)
...
!--------------------PROGRAM START--------------------------
        ...
        !Load matrix with sqrt(N) scaled rabi splittings
        allocate(coup_list(nosc-1))
        coup_list(:)=6969
        open(11,file='couplings_list.data')
                do i=1,nosc-1
                        read(11,*) coup_list(i)
                enddo
        close(11)

        write(6,*) 'The couplings are:', coup_list(:)


        !Convert now the frequencies to SI
        call conversion(into,mass_red,w_range,nbeads,nosc,pos,step,coup_list,energy_vir,energy_clas,req,de)


        !Calculate coupling const.
        coup_list(:)=mass_red*w_range(1)*coup_list(:)*4.d0*pi2**2 !Coupling constant depends on the cavity not the oscillators!!.
        !Note that the rescaling by sqrt N is done in the outside loop.
        !Now construct the matrix that contains the coupling constants

        !Action for the starting point
        call calc_action(model,nbeads,nosc,pos,b,mass_red,w_range,coup_list,pol_action,de,req)
        ...
        do i=1,cycles
                ...
                call mc_update(model,nbeads,nosc,pos,step,pol_action,accept_count,b,w_range,coup_list,mass_red,freq_coll,de,&
                ...
                call energies(model,nbeads,nosc,pos,grad,b,mass_red,w_range,coup_list,conf_energy,energy_vir,force_corr, & 
